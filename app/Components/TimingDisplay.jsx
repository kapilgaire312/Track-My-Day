...
import { updateCheckbox, updateValue, selectTimeSegment, saveActivity, outsideClick, selectionFocus } from "../utils/handleInput"
...
export default function TimingDisplay({ selectedDate }) {
  ...
  const categoryList = [
    'food', 'health', 'career', 'sleep', 'miscellaneous'
  ];

  function categoryListConvert(categoryList) {
    return categoryList.map((item) => { return { value: item, label: item } })
  ...
  useEffect(() => {
    ...
    const updateActivity = activity.map((item, index) => {
      if (item.category != categoryReturned[index]?.category)
        flag = true
      ...
      if (count < 2) {
        ...
        try {
          if (!(activity?.length === 0)) {
            (async () => {
              const categories = await saveActivityDb(activity, selectedDate.toDateString(), session.user.email, categoryList)
              console.log(categories)


              const updateActivity = activity.map((item, index) => {
                let flag = false
                if (item.category != categories[index]?.category)
                  flag = true
  ...
  useEffect(() => {
    const getActivity = async () => {
      setLoading(true)
      let storedActivity = await getActivityDb(session?.user?.email, selectedDate.toDateString())
      if (!storedActivity) {
  ...
  useEffect(() => {
    const handleClick = () => {
      const isFocused = inputRef.current.find((item) => item === document.activeElement)
      if (!isFocused) {
    ...
    <>
      <div className="mt-1 sm:mt-4">
        <div className=" grid grid-cols-[40px_50px_1fr] gap-x-4 sm:gap-x-6 gap-y-3 sm:gap-y-5 sm:grid-cols-[100px_0.5fr_1fr] select-none items-center">
          <div className="font-bold text-xl text-center "><button></button></div>
          <div className="font-semibold text-xl sm:text-2xl text-center py-1 bg-gray-300 rounded">Time</div>
          <div className="font-semibold text-xl sm:text-2xl mr-1 text-center py-1 bg-gray-300 rounded">What i did</div>
            ...
            return <div className="contents" key={index} onClick={() => {
              ...
              <div className="p-5 my-5 text-center">
                ...
                <input
                  ...
                  onChange={() => { updateCheckbox(index, activity, setActivity, inputRef) }}
              ...
              <div className="mx-4 my-5">
                {(
                  ...
                  (activity[index].category === null && activity[index].value !== '')
                )
                  && <Select
                    className="my-dropdown w-42  "

                    value={activity[index].category ? categoryListConvert([activity[index].category])[0] : null}
                    onChange={(data) => { setTimeout(() => { onSelect(data, activity, setActivity) }, 10) }}
                    placeholder="category"
                    options={categoryListConvert(categoryList)}
                    isSearchable={false}
                    onFocus={() => { selectionFocus(activity, setActivity, index) }}
                    isLoading={activity[index].category === null ? true : false}
                    ...
                    styles={{
                      ...
                      option: (provided, state) => ({
                        ...provided,
                        backgroundColor: state.isSelected ? '#cfe3ff' : state.isFocused ? '#e5f2ff' : '#ffffff',
                        color: '#111827',
                      }),
                    }}
                ...
                <textarea
                  ...
                  value={item?.value}
                  onChange={(e) => {
                    updateValue(e.target.value.trimStart(), activity, setActivity, index)
                  }}
